...
if station_count.zero?
  puts "snow_stationsテーブルにデータが存在しません"
  return
end
...
station_ids.each_with_index do |station_id, index|
  begin
    ...
    # idをキーとして最新のsnow_stationを取得
    station = SnowStation.find_by(id: station_id)

    # snowで作成した複合キーで全国版マスターと突き合わせする
    ...
    if station.nil?
      puts "ID: #{station_id} のstationが見つかりませんでした（スキップ）"
      error_count += 1
      next
    end
    ...
    if station.station_name.nil?
      puts "ID: #{station_id} のstation_nameがnilです（スキップ）"
      error_count += 1
      next
    end

    # 全国マスターの中で合致する名前が見つからない場合は飛ばして次へ
    ...
    if station.station_number != national_data[:station_number]
      ...
      if existing_station
        puts "#{station.station_name}: 観測所番号 #{new_number} が変更対応済のためスキップ"
      ...
      if ActiveRecord::Base.connection.table_exists?('user_statuses')
        ...
        if referenced_count > 0
          puts "#{station.station_name}: user_statusesによる参照件数 (#{referenced_count}件)"
